...
class ViewController: UIViewController, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        // Set very small initial size (like one letter)
        textView.frame = CGRect(x: 10, y: 100, width: 20, height: 20)
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        var newWidth = estimatedSize.width
        ...
        UIView.animate(withDuration: 0.2) {
            self.textView.frame.size = CGSize(width: newWidth, height: newHeight)
        }
    }
}
    ...
    func wasDragged(gestureRecognizer: UIPanGestureRecognizer) {
        ...
        if gestureRecognizer.state == .ended {
            ...
            if draggedView.frame.intersects(btnDelete.frame) {
                ...
                print("🗑️ Removed item by dragging into delete area")
            }
...
class ViewController: UIViewController, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        textView.delegate = self
        textView.isScrollEnabled = false // So it grows instead of scrolling
    }
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        // Define maximum width and height (-10 from screen size)
        let maxWidth = UIScreen.main.bounds.width - 10
        let maxHeight = UIScreen.main.bounds.height - 10
        
        // Final width and height (whichever is smaller)
        let finalWidth = min(newSize.width, maxWidth)
